...
describe("visibleLetterStatus", () => {
  it("solo colorea letras ausentes de todos los tableros en modo general", () => {
  ...
  it("muestra únicamente el estado del tablero seleccionado", () => {
...
describe("letterKeyLabel", () => {
  it("explica el descarte global sin deshabilitar la tecla", () => {
    expect(letterKeyLabel("B", "absent", null)).toBe(
      "Letra B: no está en ninguna palabra",
    );
  });

  it("identifica el tablero filtrado", () => {
    expect(letterKeyLabel("A", "present", 1)).toBe(
      "Letra A: presente sin posición en el tablero 2",
    );
  });
});
